        ...
        <ContentTemplate>
            ...
            <asp:GridView ID="CustomerListGridView" runat="server" AutoGenerateColumns="False" AllowPaging="true" PageSize="100" AllowSorting="true" CssClass="mGrid" DataKeyNames="CustomerID" DataSourceID="CustomerListSQL" OnRowDeleted="CustomerListGridView_RowDeleted">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="বিবরণ" SortExpression="Description">
                        <EditItemTemplate>
                            <asp:TextBox ID="DescriptionTextBox" CssClass="textbox"  runat="server" Text='<%# Bind("Description") %>'></asp:TextBox>
                        </EditItemTemplate>
                        <ItemTemplate>
                            <asp:Label ID="DescriptionLabel3" runat="server" Text='<%# Bind("Description") %>'></asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
            ...
            <asp:SqlDataSource ID="CustomerListSQL" runat="server" ConnectionString="<%$ ConnectionStrings:TailorBDConnectionString %>"
                SelectCommand="SELECT Customer_Due,Description, CustomerID, RegistrationID, InstitutionID, Cloth_For_ID, CustomerNumber, CustomerName, Phone, Address, Image, Date  FROM Customer WHERE (InstitutionID = @InstitutionID) AND (Phone LIKE '%' + @Phone + '%') AND (CustomerNumber = @CustomerNumber OR @CustomerNumber = 0) AND (ISNULL(CustomerName, N'') LIKE '%' + @CustomerName + '%') ORDER BY Date DESC"
                OnSelected="CustomerListSQL_Selected"
                DeleteCommand="DELETE FROM Customer WHERE (CustomerID = @CustomerID)"
                UpdateCommand="UPDATE Customer SET CustomerName = @CustomerName,Description=@Description, Phone = @Phone, Address = @Address WHERE (CustomerID = @CustomerID)">
