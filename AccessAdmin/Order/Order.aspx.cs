    ...
    public partial class Order : System.Web.UI.Page
    {
        ...
        protected void AddButton_Click(object sender, EventArgs e)
        {
            ...
            if (Is_Customer == null)
            {
                ...
                Response.Redirect("~/AccessAdmin/Order/Dressandmeasurements.aspx?CustomerID=" + Customerid + "&Cloth_For_ID=" + GenderDropDownList.SelectedValue);
            }
            else
            {
        ...
        public static string CheckMobileNo(string MobileNo, string ID)
        {
            ...
            SqlCommand cmd = new SqlCommand("select Top(1) CustomerID from Customer where InstitutionID = @InstitutionID AND Phone = @Phone", con);
        ...
        public static string Check_Name_Mobile(string Name, string Mobile, string ID)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TailorBDConnectionString"].ToString());
            string retval = "";
            SqlCommand cmd = new SqlCommand("select Top(1) * from Customer where InstitutionID = @InstitutionID AND CustomerName = @CustomerName AND Phone = @Phone", con);
            cmd.Parameters.AddWithValue("@InstitutionID", ID);
