...
(function () {
    ...
    function loadData() {
        $.ajax({
            url: `/api/ItemCategory?institutionId=${institutionId}`,
            method: 'GET',
    ...
    function renderTable(rows) {
        ...
        rows.forEach(function (row, i) {
            $body.append(`
            <tr id="row-${row.ItemCategoryID}">
                <td class="serial-no">${i + 1}</td>
                <td id="name-cell-${row.ItemCategoryID}"><span>${escHtml(row.CategoryName)}</span></td>
                <td>
                    <div class="action-btns" id="action-${row.ItemCategoryID}">
                        <button class="btn-edit" onclick="startEdit(${row.ItemCategoryID},'${escHtml(row.CategoryName)}')">
                            <i class="fas fa-pen"></i> ${lang === 'en' ? 'Edit' : 'এডিট'}
                        </button>
                        <button class="btn-delete" onclick="askDelete(${row.ItemCategoryID},'${escHtml(row.CategoryName)}')">
    ...
    function showEmptyRow() {
        const lang = window.currentLang === 'en' ? 'en' : 'bn';
        $('#tableBody').html(`<tr class="empty-row"><td colspan="3"><i class="fas fa-inbox me-2" style="color:#ccc"></i>${lang === 'en' ? 'No categories added yet' : 'কোনো ক্যাটাগরি যুক্ত হয়নি'}</td></tr>`);
        $('#rowCount').text(0);
    }

    window.addCategory = function () {
        const val = $('#categoryNameInput').val().trim();
        $('#categoryNameInput').removeClass('is-invalid'); $('#categoryNameError').text('');
        if (!val) {
            $('#categoryNameInput').addClass('is-invalid');
            $('#categoryNameError').text(window.currentLang === 'en' ? 'Please enter category name' : 'ক্যাটাগরির নাম দিন');
            $('#categoryNameInput').focus(); return;
        }
        const $btn = $('#btnAdd');
        $btn.prop('disabled', true).html('<span class="spinner-sm"></span>');
        $.ajax({
            url: '/api/ItemCategory', method: 'POST', contentType: 'application/json',
            data: JSON.stringify({ categoryName: val, institutionID: institutionId, registrationID: registrationId }),
            success: function (res) {
                if (res.success) { $('#categoryNameInput').val(''); showAlert(res.message, 'success'); loadData(); }
    ...
    window.saveEdit = function (id) {
        ...
        $.ajax({
            url: `/api/ItemCategory/${id}`, method: 'PUT', contentType: 'application/json',
            data: JSON.stringify({ categoryName: val, institutionID: institutionId, registrationID: registrationId }),
    ...
    window.askDelete = function (id, name) { deleteTargetId = id; $('#deleteItemName').text(name); deleteModal.show(); };

    function confirmDelete(id) {
        $.ajax({
            url: `/api/ItemCategory/${id}`, method: 'DELETE',
