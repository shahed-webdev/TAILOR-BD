...
(function () {
    ...
    function loadData() {
        $.ajax({
            url: `/api/ItemBrand?institutionId=${institutionId}`,
            method: 'GET',
    ...
    function renderTable(rows) {
        ...
        rows.forEach(function (row, i) {
            $body.append(`
            <tr id="row-${row.ItemBrandID}">
                <td class="serial-no">${i + 1}</td>
                <td id="name-cell-${row.ItemBrandID}"><span>${escHtml(row.BrandName)}</span></td>
                <td>
                    <div class="action-btns" id="action-${row.ItemBrandID}">
                        <button class="btn-edit" onclick="startEdit(${row.ItemBrandID},'${escHtml(row.BrandName)}')">
                            <i class="fas fa-pen"></i> ${lang === 'en' ? 'Edit' : 'এডিট'}
                        </button>
                        <button class="btn-delete" onclick="askDelete(${row.ItemBrandID},'${escHtml(row.BrandName)}')">
    ...
    function showEmptyRow() {
        const lang = window.currentLang === 'en' ? 'en' : 'bn';
        $('#tableBody').html(`<tr class="empty-row"><td colspan="3"><i class="fas fa-inbox me-2" style="color:#ccc"></i>${lang === 'en' ? 'No brands added yet' : 'কোনো ব্র্যান্ড যুক্ত হয়নি'}</td></tr>`);
        $('#rowCount').text(0);
    }

    window.addBrand = function () {
        const val = $('#brandNameInput').val().trim();
        $('#brandNameInput').removeClass('is-invalid'); $('#brandNameError').text('');
        if (!val) {
            $('#brandNameInput').addClass('is-invalid');
            $('#brandNameError').text(window.currentLang === 'en' ? 'Please enter brand name' : 'ব্র্যান্ডের নাম দিন');
            $('#brandNameInput').focus(); return;
        }
        const $btn = $('#btnAdd');
        $btn.prop('disabled', true).html('<span class="spinner-sm"></span>');
        $.ajax({
            url: '/api/ItemBrand', method: 'POST', contentType: 'application/json',
            data: JSON.stringify({ brandName: val, institutionID: institutionId, registrationID: registrationId }),
            success: function (res) {
                if (res.success) { $('#brandNameInput').val(''); showAlert(res.message, 'success'); loadData(); }
    ...
    window.saveEdit = function (id) {
        ...
        $.ajax({
            url: `/api/ItemBrand/${id}`, method: 'PUT', contentType: 'application/json',
            data: JSON.stringify({ brandName: val, institutionID: institutionId, registrationID: registrationId }),
    ...
    function confirmDelete(id) {
        $.ajax({
            url: `/api/ItemBrand/${id}`, method: 'DELETE',
